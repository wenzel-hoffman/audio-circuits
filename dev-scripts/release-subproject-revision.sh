...
# A versions of “release-revision.sh” that allows to release a subproject
# under one big project directory.
#
# Later I might merge both scripts together.
...
>/dev/null type column

SCRIPT_DIR=$(dirname -- "${BASH_SOURCE[0]}")

if (( $# != 3 )) || [[ -z $1 ]] || [[ -z $2 ]] || [[ ! $3 =~ r[0-9]+ ]]; then
  >&2 echo 'Usage:'
  (
    printf '%s SUBPROJECT RELEASE_SUB_NAME REVISION\n' "${0%Q}"
    printf '%s wenzels-powered-guitar-cabinet-2602 2602-cabinet r1\n' "${0%Q}"
    printf '%s wenzels-powered-guitar-cabinet-2602.kicad_pro 2602-cabinet r1\n' "${0%Q}"
    printf '%s wenzels-powered-guitar-cabinet-2602.kicad_sch 2602-cabinet r1\n' "${0%Q}"
  ) | column -t | (while read -r x; do printf '  %s\n' "$x"; done) >&2
  >&2 echo 'Which will create a directory like “release-2602-cabinet-2026-03-r1”.'
  exit 1
fi

SUBPROJECT=$1; shift
SUBPROJECT=$(basename -- "$SUBPROJECT" .kicad_pro)
SUBPROJECT=$(basename -- "$SUBPROJECT" .kicad_sch)

RELEASE_SUB_NAME=$1; shift
...
RELEASE_DIR="release-$RELEASE_SUB_NAME-$DATE-$REVISION"
README_FILE=$RELEASE_DIR/README.md

>&2 printf "RELEASE_DIR='%s'\n" "$RELEASE_DIR"
>&2 printf "README_FILE='%s'\n" "$README_FILE"

KICAD_PROJECT_FILES=$(find . -maxdepth 1 -name "$SUBPROJECT.kicad_pro" -printf '%f\n')
